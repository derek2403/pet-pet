...
export const drawPetWithActivity = (ctx, dog, activity, color = '#EF4444') => {
  // Always use red color
  const redColor = '#EF4444';
  drawBoundingBox(ctx, dog.bbox, redColor, 3);
  
  const label = dog.class === 'person' ? 'PET' : dog.class.toUpperCase();
  
  // Remove emoji and confidence - just show label and activity text
  const activityText = activity.replace(/[\u{1F300}-\u{1F9FF}]/gu, '').trim();
  const text = `${label} - ${activityText}`;
  
  drawLabel(ctx, text, dog.bbox[0], dog.bbox[1], redColor, 20);
};
